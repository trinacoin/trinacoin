...
#if defined(NDEBUG)
# error "Trinacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9e460ea09481c762475270f11e5a114643c2499e7584288a8a51a6ce78d0f3e8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Trinacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Trinacoin Signed Message:\n";
...
{
    // Trinacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 400000 blocks, which will occur approximately every 1.5 years
    nSubsidy >>= (nHeight / 400000); // Trinacoin: 400k blocks in ~1.5 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Trinacoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Trinacoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Trinacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Trinacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc1f1e9ca50cf288417e397f6dc0aff0cfbd8dbc5ddd46bb311389e4a1376a848");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Italy suggest to impose tax on savings concealed in bank's safety deposits - FinanceWhile.com - 13 June 2019";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 40 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045fe5a1f37e74c471190e92127833ed484fc6a568f4efddbed6d0dddf7e8072d9d4a6124e8196ef795d40f1d98c319e12f7b62374ac115a3c06484e5a32cd67ce") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1560456446;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084918713;

        if (fTestNet)
        {
            block.nTime    = 1560456358;
            block.nNonce   = 387639800;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x951d4c02589099b2a4f3009f9e585918838bf1310843254d484d106fc340d227"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xca, 0xbb, 0xee, 0x22 }; // Trinacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TrinacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TrinacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TrinacoinMiner(CWallet *pwallet)
{
    printf("TrinacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("trinacoin-miner");
    ...
    try { loop {
        ...
        printf("Running TrinacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TrinacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TrinacoinMiner, pwallet));
}
